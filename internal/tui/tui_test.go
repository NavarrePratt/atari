...
func TestNew_WithWorkingDirectory(t *testing.T) {
	eventChan := make(chan events.Event)

	tui := New(eventChan, WithWorkingDirectory("/path/to/workdir"))

	if tui.workingDirectory != "/path/to/workdir" {
		t.Errorf("expected workingDirectory '/path/to/workdir', got %q", tui.workingDirectory)
	}
}
